...
class GeminiService {
  ...
  async generateGeminiPost(
    ...
    try {
      ...
      console.log(`🚀 Generating ${variantCount} posts with Gemini (Model: ${this.availableModels[this.currentModelIndex]}) and framework: ${framework || 'auto'}`);
      ...
      if (this.currentModelIndex < this.availableModels.length - 1) {
        ...
        return this.generateGeminiPost(
          documentContent,
          platform,
          tone,
          variantCount,
      ...
      // If Gemini fails completely, try Groq if available
      if (this.groqAI) {
  ...
  async generateGroqPost(
    ...
    if (!this.groqAI) throw new Error("Groq is not initialized");
    ...
    try {
      ...
      console.log(`🚀 Generating ${variantCount} posts with Groq (openai/gpt-oss-20b) and framework: ${framework || 'auto'}`);
...
- It should sound like a real person is talking directly to their LinkedIn audience — with warmth, personality, and natural flow.
  ...
  async generateWithRetry(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          "Attempt " + (attempt + 1) + ": Generated " + (posts?.length || 0) + " posts. Retrying..."
        );
      } catch (error: unknown) {
  ...
  async testConnection(): Promise<boolean> {
    if (!this.genAI) {
      console.warn("⚠️ Gemini is not initialized, skipping connection test.");
      return false;
    }
    try {
