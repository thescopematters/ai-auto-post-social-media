...
router.post(
  ...
  validate([
    ...
    body('variantCount').optional().isInt({ min: 1, max: 20 }).withMessage('Variant count must be between 1 and 20'),
    body('agentConfigId').optional().isUUID().withMessage('agentConfigId must be a valid UUID'),
  ]),
  requireWorkspace,
