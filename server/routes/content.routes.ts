...
router.post(
  "/:workspaceId/posts/:postId/upload-image",
  upload.single("image"),
  validate([
    param("workspaceId").isUUID().withMessage("Invalid workspace ID"),
    param("postId").isUUID().withMessage("Invalid post ID"),
  ...
  requireWorkspace,
  requireRole(["admin", "editor"]),
  contentController.uploadPostImage
);

router.delete(
  "/:workspaceId/posts/:postId/remove-image",
  validate([
    param("workspaceId").isUUID().withMessage("Invalid workspace ID"),
    param("postId").isUUID().withMessage("Invalid post ID"),
  ]),
  requireWorkspace,
  requireRole(["admin", "editor"]),
  contentController.removePostImage
);
